...
public class ShoppingCart {
    ...
    public ShoppingCart(CommandHistory commandHistory) {
    ...
    public BigDecimal calculatePrice() {
        ...
        for (var discount : discounts) {
            var totalDiscount = discount.getDiscount(items);
    ...
    public void undo() {
        int index = commandHistory.getUndoList().size() - 1;
        commandHistory.getUndoList().get(index).execute();
        commandHistory.getRedoList().push(commandHistory.getUndoList().peek());
        commandHistory.getUndoList().pop();
    }

    public void redo() {
        if (commandHistory.getRedoList().size() > 0) {
            commandHistory.getRedoList().get(0).redo();
        }
    }
}
